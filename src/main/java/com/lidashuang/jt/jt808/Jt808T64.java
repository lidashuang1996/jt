 ...
 * 平台RSA公钥消息体数据格式
 * 起始字节  字段     数据类型    描述及要求
 * 0        e       DWORD       平台 RSA 公钥{e,n}中的 e
 * 4        n       BYTE[128]   RSA 公钥{e,n}中的 n
 *
 */
public class Jt808T64 {
    /** 消息 ID：0x8A00 */
    public static final int M_ID = 0x8A00;
    ...
    public Jt808T64(byte[] bytes) {
        if (bytes != null && bytes.length == M_LEN) {
            ...
            throw new RuntimeException("数据格式 --> 平台 RSA 公钥 --> 长度出现问题！");
        }
    }
